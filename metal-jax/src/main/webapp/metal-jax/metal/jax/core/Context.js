//@ sourceURL=metal.jax.core.Context
/**
...
$static.getName = function getName() { return "metal.jax.core.Context"; };
$private.Context = function Context() { return $protected.call(this, arguments); };
$imports["BaseObject"] = "metal.jax.BaseObject";
$imports["System"] = "metal.jax.core.System";
$imports["Logger"] = "metal.jax.core.Logger";
$imports["metal.jax.resource.Messages"] = "metal.jax.resource.Messages";
$imports["metal.jax.resource.Properties"] = "metal.jax.resource.Properties";
$requires["metal.jax-messages"] = "metal.jax.resource.Messages";
$requires["metal.jax-conf"] = "metal.jax.resource.Properties";
...
$private.initContext = function initContext(scope) {
	...
	if ($setting["baseModule"] == ".") {
		log("warn", "metal.jax.init.baseModule.default", $setting);
	}
	if (BaseContext) {
		log("info", "metal.jax.init.baseContext", $setting);
	} else {
		log("warn", "metal.jax.init.baseContext.fail", $setting);
	}
	log("info", "metal.jax.init.baseResource", $setting);
	...
	if (match) {
		...
		while (match = _classSettingRE_.exec(header)) {
			...
			case "requires":
				...
				if (name) {
					source.scope.$imports[parseBaseName(name)] = name;
